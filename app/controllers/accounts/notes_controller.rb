...
module Accounts
  class NotesController < Accounts::BaseController
    ...
    before_action :find_note, only: %i[show complete reopen]
    before_action :find_notes, only: %i[index completed]

    def index
      @notes = @notes.where(state: %w[new reopened])
    end
    ...
    def completed
      @notes = @notes.where(state: %w[accepted rejected])
